...
def ai_control():
    ...
    for enemy in enemy_group:
        distance = abs(enemy.rect.x - p.rect.x)
        if distance < min_distance:
            min_distance = distance
            closest_enemy = enemy
    
    # Move towards the closest enemy's x position
    if closest_enemy:
        if p.rect.x < closest_enemy.rect.x:
            moving_left = False
            moving_right = True
        elif p.rect.x > closest_enemy.rect.x:
            moving_left = True
            moving_right = False
        else:
            moving_left = False
    ...
    # Shoot if the enemy is in front of the player
    if closest_enemy and abs(p.rect.x - closest_enemy.rect.x) < 50:
        shoot_bullet()
...
while running:
	...
	if game_page:
		ai_control()  # Call AI control function instead of handling player input manually
